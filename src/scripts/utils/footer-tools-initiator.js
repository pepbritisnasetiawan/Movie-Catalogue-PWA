...
const FooterToolsInitiator = {
  ...
  // melakukan proses subscribe ke push message
  async _subscribePushMessage(event) {
    ...
    try {
      ...
      await pushSubscription?.unsubcribe();
    }

    this._showSubscribeButton();
  },

  // melakukan proses unsubscribe ke push message
  async _unsubscribePushMessage(event) {
    ...
    try {
      ...
      console.log('_subscribePushMessage:', pushSubscription);
    } catch (err) {
      console.error(
  ...
  // membuat HTTP Request dengan method POST
